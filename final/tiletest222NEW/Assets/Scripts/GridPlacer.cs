...
public class GridPlacer : MonoBehaviour
{
    ...
    {
        ClearGrid();
        currentGrid = JsonConvert.DeserializeObject<float[,]>(jsonGrid);
        if (currentGrid == null)
        {
        ...
        PlaceTiles(currentGrid);
        if (cameraController != null)
            cameraController.AdjustCamera(currentGrid); // Adjust camera after generating the grid
        else
