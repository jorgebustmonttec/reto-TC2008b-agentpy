...
{
    public List<GameObject> carPrefabs; // Now it's a party
    public List<GameObject> rareCarPrefabs; // The VIP section
    ...
    private float frameRate = 0.2f; // 10 fps
        ...
        foreach (var frame in frames)
        {
            ...
            foreach (var carInfo in frame.cars)
            {
                ...
                else
                {
                    ...
                    GameObject newCar;
                    float roll = Random.Range(0f, 1f); // Roll the dice

                    if (roll < 0.05f && rareCarPrefabs.Count > 0)
                    { // 5% chance for rare car
                        newCar = Instantiate(rareCarPrefabs[Random.Range(0, rareCarPrefabs.Count)], position, rotation);
                    }
                    else
                    {
                        newCar = Instantiate(carPrefabs[Random.Range(0, carPrefabs.Count)], position, rotation);
                    }

                    // Set the car's name to its ID, assuming you still want to track them
                    newCar.name = carInfo.id;
    ...
    Quaternion DirectionToRotation(int direction)
    {
        ...
        switch (direction)
        {
            ...
            case 3: angle = 270; break;  // East
            case 4: angle = 90; break; // West
